...
# compile a custom bf_switchd agent to the local build directory.
function build_mgr() {
...
# compile a bf_switchd agent to the local build directory -- public function.
function build_control() {
    CTL_SRC=$(to_switchd_fn "$1")
    BUILD_DIR=$(to_build_dir "$1")
    build_mgr "$CTL_SRC" "$BUILD_DIR"
}
...
function runhw() {
    trap 'stop_switchd' 9
    starthw $1 $2 # pass second argument to python
    # wait for ctl + c or exit
...
# run a test on an already running simulator
function test_on_running_sim() {
    local jsonfn="$1"
    local pcapfn="trace.pcap"
    # figure out port to send packets out of
    local dpid_in=$(python3 $SCRIPT_DIR/testspec_utils.py input_port "$jsonfn")   
...
function main() {
    ...
    case $1 in 
        "build") shift; build $@
        ;;
        "build_control") shift; build_control $@
        ;;
        ...
        "test") shift; test $@
        ;;
        "test_on_running_sim") shift; test_on_running_sim $@
        ;;
        ...
        *) echo "usage:"; 
           ...
           echo "p4tapp.sh test_on_running_sim testspec.json"
