...
class Manager(object):
  ...
  def port_up(self, dpid, rate, fec_type):  
    ...
    rate = self.bw_dict[rate]
    fec_type = self.fec_dict[fec_type]
  ...
  def setDefaultAction(self, tableName, actionName, actionArgs={}):
    print ("setting default action: {0} --> {1}({2})".format(tableName, actionName, actionArgs))
    bfrt_info = self.interface.bfrt_info_get(self.p4Name)
    target = gc.Target(device_id=0, pipe_id=0xffff)
    table = bfrt_info.table_get(tableName)    
    action_data = table.make_data(
      [gc.DataTuple(argName, argVal) for argName, argVal in actionArgs.items()],
      actionName
    ) 
    table.default_entry_set(target, action_data)
  ...
  def addExactEntry(self, tableName, fieldNames, fieldVals, actionName, actionArgs={}):
    ...
    # make a key out of a list of key tuples, then put that in a list.
    key_list = []    
    key_list = [table.make_key(
      [gc.KeyTuple(fieldName, fieldVal) for (fieldName, fieldVal) in zip(fieldNames, fieldVals)]
      )]
    data_list = []
